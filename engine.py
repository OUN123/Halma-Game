                ...
                while (self.terminate_state() == 0):
                    ...
                    if (self.turn == 2):
                        ...
                        if (self.system == "CMD"):
                            chosen = False
                            while (not(chosen)):
                                pion = input("Choose your pion. Write in [x,y] without [] ")
                            
                                if not(re.search("[0-9][,][0-9]", pion) == None):
                                    fromx, fromy = pion.split(",")
                                    fromx = int(fromx)
                                    fromy = int(fromy)
                                    if (not(player.isExist_pions(fromx, fromy))):
                                        pions = ''
                                        for i in range (len(player.pions)):
                                            pions += "(" + str(player.pions[i].x) + "," + str(player.pions[i].y) + ") "
                                        print("pion not available in current coordinate! Available pions :", pions)
                                    elif (len(self.board.getMoveFromTile(player, fromx, fromy)) == 0):
                                        pions = ''
                                        for i in range (len(player.pions)):
                                            if (player.pions[i].x != fromx and player.pions[i].y != fromy):
                                                pions += "(" + str(player.pions[i].x) + "," + str(player.pions[i].y) + ") "
                                        print("No move available, please choose other pion! Available pions :", pions)
                                    else:
                                        chosen = True
                            ...
                            while not(moved):
                                ...
                                if (not(re.search("[0-9][,][0-9]", move) == None)):
                                    ...
                                    if ((to_x, to_y) in self.board.getMoveFromTile(player, fromx, fromy)):
                                        ...
                                        self.board.movepion((fromy, fromx), (to_y, to_x))
                                        print()
                                        moved = True
                                    else:
                                        print("Invalid move!")
...
class BoardGUI(tk.Tk):
    ...
    def drawpion(self):
        ...
        border_size = 10
        cell = int(canvas_height / self.board_size)
        self.player1pion = self.board.player1.pions
        self.player2pion = self.board.player2.pions
        # delete previous pions canvas
        self.canvas.delete('pion')

        for i in range(len(self.player1pion)):
            col = self.player1pion[i].x - 1
            row = self.player1pion[i].y - 1 
            x1 = col * cell + border_size / 2
            y1 = row * cell + border_size / 2
            x2 = (col + 1) * cell - border_size / 2
            y2 = (row + 1) * cell - border_size / 2
            if (self.board.player1.color == "GREEN"):
                pion = self.canvas.create_oval(x1, y1, x2, y2, tags="pion", width=0, fill="#67BF9B")
            else:
                pion = self.canvas.create_oval(x1, y1, x2, y2, tags="pion", width=0, fill="#CF6E67")
            self.canvas.tag_bind(pion, "<1>", lambda event, row=row, col=col: self.clicked(row+1, col+1))

        for i in range(len(self.player2pion)):
            col = self.player2pion[i].x - 1
            row = self.player2pion[i].y - 1 
            x1 = col * cell + border_size / 2
            y1 = row * cell + border_size / 2
            x2 = (col + 1) * cell - border_size / 2
            y2 = (row + 1) * cell - border_size / 2
            if (self.board.player1.color == "GREEN"):
                ...
                pion = self.canvas.create_oval(x1, y1, x2, y2, tags="pion", width=0, fill="#67BF9B")
            self.canvas.tag_bind(pion, "<1>")

        # update pion's coordinate everytime it moves
        self.update()
    ...
    def clicked(self, row, column):
        ...
        if (self.board.selected_tuple == None and self.board.player1.isExist_pions(column, row)):
            if (self.board.player1.isExist_pions(column, row)):
                pion = self.board.player1.getpion(column, row)
            elif (self.board.player2.isExist_pions(column, row)):
                pion = self.board.player2.getpion(column, row)

            validMoves = self.board.getAksiValid(pion)
        ...
        elif (self.board.selected_tuple != None and (column, row) in self.board.getAksiValid(self.board.player1.getpion(self.board.selected_tuple[0], self.board.selected_tuple[1]))):
            (x,y) = self.board.selected_tuple
            self.board.movepion((y, x), (row, column))
            for i in range (self.board_size):
